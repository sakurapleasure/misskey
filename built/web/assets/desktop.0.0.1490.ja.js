...
/***/ (function(module, exports, __webpack_require__) {
    ...
    var riot = __webpack_require__(0)
    riot.tag2('mk-post-form-window', '<mk-window ref="window" is-modal="{true}"><yield to="header"><span if="{!parent.opts.reply}">新規投稿</span><span if="{parent.opts.reply}">返信</span><span class="files" if="{parent.files.length != 0}">{\'添付: {}メディア\'.replace(\'{}\', parent.files.length)}</span><span class="uploading-files" if="{parent.uploadingFiles.length != 0}">{\'{}つのメディアをアップロード中\'.replace(\'{}\', parent.uploadingFiles.length)}<mk-ellipsis></mk-ellipsis></span></yield><yield to="content"><div class="ref" if="{parent.opts.reply}"><mk-post-preview post="{parent.opts.reply}"></mk-post-preview></div><div class="body"><mk-post-form ref="form" reply="{parent.opts.reply}"></mk-post-form></div></yield></mk-window>', 'mk-post-form-window > mk-window [data-yield=\'header\'] > .files,[data-is="mk-post-form-window"] > mk-window [data-yield=\'header\'] > .files,mk-post-form-window > mk-window [data-yield=\'header\'] > .uploading-files,[data-is="mk-post-form-window"] > mk-window [data-yield=\'header\'] > .uploading-files{margin-left:8px;opacity:.8;} mk-post-form-window > mk-window [data-yield=\'header\'] > .files:before,[data-is="mk-post-form-window"] > mk-window [data-yield=\'header\'] > .files:before,mk-post-form-window > mk-window [data-yield=\'header\'] > .uploading-files:before,[data-is="mk-post-form-window"] > mk-window [data-yield=\'header\'] > .uploading-files:before{content:\'(\'} mk-post-form-window > mk-window [data-yield=\'header\'] > .files:after,[data-is="mk-post-form-window"] > mk-window [data-yield=\'header\'] > .files:after,mk-post-form-window > mk-window [data-yield=\'header\'] > .uploading-files:after,[data-is="mk-post-form-window"] > mk-window [data-yield=\'header\'] > .uploading-files:after{content:\')\'} mk-post-form-window > mk-window [data-yield=\'content\'] > .ref > mk-post-preview,[data-is="mk-post-form-window"] > mk-window [data-yield=\'content\'] > .ref > mk-post-preview{margin:16px 22px}', '', function(opts) {
		this.uploadingFiles = [];
		this.files = [];
...
riot.tag2('mk-repost-form', '<mk-post-preview post="{opts.post}"></mk-post-preview><virtual if="{!quote}"><footer><a class="quote" if="{!quote}" onclick="{onquote}">引用する...</a><button class="cancel" onclick="{cancel}">キャンセル</button><button class="ok" onclick="{ok}" disabled="{wait}">{wait ? \'しています...\' : \'Repost\'}</button></footer></virtual><virtual if="{quote}"><mk-post-form ref="form" repost="{opts.post}"></mk-post-form></virtual>', 'mk-repost-form > mk-post-preview,[data-is="mk-repost-form"] > mk-post-preview{margin:16px 22px} mk-repost-form > div,[data-is="mk-repost-form"] > div{padding:16px} mk-repost-form > footer,[data-is="mk-repost-form"] > footer{height:72px;background:#f9fcf4;} mk-repost-form > footer > .quote,[data-is="mk-repost-form"] > footer > .quote{position:absolute;bottom:16px;left:28px;line-height:40px} mk-repost-form > footer button,[data-is="mk-repost-form"] > footer button{display:block;position:absolute;bottom:16px;cursor:pointer;padding:0;margin:0;width:120px;height:40px;font-size:1em;outline:none;border-radius:4px;} mk-repost-form > footer button:focus:after,[data-is="mk-repost-form"] > footer button:focus:after{content:"";pointer-events:none;position:absolute;top:-5px;right:-5px;bottom:-5px;left:-5px;border:2px solid rgba(135,187,53,0.3);border-radius:8px} mk-repost-form > footer > .cancel,[data-is="mk-repost-form"] > footer > .cancel{right:148px;color:#888;background:linear-gradient(to bottom,#fff 0%,#f5f5f5 100%);border:solid 1px #e2e2e2;} mk-repost-form > footer > .cancel:hover,[data-is="mk-repost-form"] > footer > .cancel:hover{background:linear-gradient(to bottom,#f9f9f9 0%,#ececec 100%);border-color:#dcdcdc} mk-repost-form > footer > .cancel:active,[data-is="mk-repost-form"] > footer > .cancel:active{background:#ececec;border-color:#dcdcdc} mk-repost-form > footer > .ok,[data-is="mk-repost-form"] > footer > .ok{right:16px;font-weight:bold;color:#fff;background:linear-gradient(to bottom,#a6d261 0%,#95c942 100%);border:solid 1px #9bcc4c;} mk-repost-form > footer > .ok:hover,[data-is="mk-repost-form"] > footer > .ok:hover{background:linear-gradient(to bottom,#92c83d 0%,#7cac31 100%);border-color:#87bb35} mk-repost-form > footer > .ok:active,[data-is="mk-repost-form"] > footer > .ok:active{background:#87bb35;border-color:#87bb35}', '', function(opts) {
		...
		this.ok = () => {
			...
			this.api('posts/create', {
				...
				__webpack_require__.i(__WEBPACK_IMPORTED_MODULE_0__scripts_notify__["a" /* default */])('Repostできませんでした');
			}).then(() => {
				this.update({
					wait: false
				});
